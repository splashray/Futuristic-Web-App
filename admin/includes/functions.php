...
function insert_about(){
    ...
    if(isset($_POST['add-about'])){
        ...
        move_uploaded_file($post_image_temp, "./images/about/$post_image");
    if($title==""||$body==""||$post_image==""){
        echo "<script type='text/javascript'> document.location = 'about-us.php'; </script>";
    }else{
            $query = "INSERT INTO about(title,body,images) " ;
            $query.= "VALUE('{$title}','{$body}','{$post_image}' ) " ;
